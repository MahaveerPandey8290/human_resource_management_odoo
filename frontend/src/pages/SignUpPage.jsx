...
export default function SignUpPage() {
  ...
  return (
    <AuthLayout cardMaxWidth="max-w-6xl">
      <div className="w-full max-w-xl mx-auto">
        {registeredData ? (
          /* System Generated Login ID Screen */
          <div className="flex flex-col gap-5">
            <div className="flex gap-3 items-start bg-success-tint rounded-2xl px-4 py-3.5 border border-success/20">
              <ShieldCheck className="w-6 h-6 text-success shrink-0 mt-0.5" />
              <div>
                <h2 className="text-h3 font-bold text-ink-primary">Company Registered!</h2>
                <p className="text-small text-ink-secondary mt-0.5">
                  Your company account for <span className="font-semibold text-ink-primary">{registeredData.company?.name || form.companyName}</span> is ready.
                </p>
              </div>
            </div>

            {/* System Generated Login ID Box */}
            <div className="bg-sunken border border-border-strong rounded-2xl p-5 shadow-xs">
              <p className="text-label uppercase tracking-wider text-ink-muted mb-2 font-semibold">
                Your System Generated Login ID
              </p>
              <div className="flex items-center justify-between gap-3 bg-white border border-border rounded-xl px-3.5 py-3 shadow-xs">
              ...
              <div className="mt-3.5 pt-3.5 border-t border-border flex flex-col gap-1.5 text-xs text-ink-secondary">
                ...
                <div className="grid grid-cols-2 gap-2 text-ink-muted mt-1">
            ...
            {/* Info Note */}
            <div className="flex gap-2.5 items-start bg-info-tint border border-info/20 rounded-xl px-3.5 py-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-2">
              <Button onClick={() => navigate('/employees')} className="flex-1 flex items-center justify-center gap-2">
                <span>Go to Dashboard</span>
                <ArrowRight className="w-4 h-4" />
              </Button>
              <Button variant="ghost" onClick={() => navigate('/signin')} className="flex-1">
                Sign In Page
              </Button>
            </div>
          </div>
          ...
          <>
            <div className="mb-6">
              <h1 className="text-h2 sm:text-h1 font-bold text-ink-primary tracking-tight">Create your company</h1>
              <p className="text-body text-ink-secondary mt-1.5">
                Register your organization and receive your system-generated Login ID.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <label className="flex items-center gap-3 p-3 border-2 border-dashed border-border-strong rounded-xl cursor-pointer hover:border-primary/50 hover:bg-sunken/50 transition-colors">
                  <div className="w-10 h-10 rounded-lg bg-primary-tint flex items-center justify-center overflow-hidden shrink-0">
                    {logo ? <img src={logo} alt="" className="w-full h-full object-cover" /> : <UploadCloud className="w-5 h-5 text-primary" />}
                  </div>
                  <span className="text-small text-ink-secondary">{logo ? 'Logo selected' : 'Click to upload logo (optional)'}</span>
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Input label="Company name" placeholder="Acme Corp" icon={Building2} value={form.companyName} onChange={(e) => set('companyName', e.target.value)} error={errors.companyName} />
